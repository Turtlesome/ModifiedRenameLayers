...
function replacements( gd, string ) 
{
  ...
  // Info relating to the active layer to the current instance of Photoshop.
  ...
  for (var i = 0; i < r.length; i++) 
  {
    if ( string.match( r[i] ) !== null ) 
    {
      ...
      string = string.split( r[i] ).join( v );  // Replace all r[i] with v
    }
...
function createDialog( previousLayerNames, textFilePath ) 
{
  ...
  */
  ...
  var layerNameInput = group2.add("edittext");
      ...
      layerNameInput.preferredSize.width = 300;
  
  var dropdown = group2.add("dropdownlist");
      dropdown.text = "Recently used names:";
  loadItemsFromFile(textFilePath, function (items) 
  {
    dropdown.removeAll();
  ...
  {
    var selectedOption = dropdown.selection.text;
    layerNameInput.text = selectedOption;
  };
  ...
  var characterCountLabel = group1.add("statictext", undefined, "Character Count: 0");
  characterCountLabel.justify = "center";


  var checkbox1 = group1.add("checkbox", undefined, "Do you want to rename all of the sub-layers?");
  checkbox1.onClick = function () 
  {
    renameAllSublayers = checkbox1.value;
  };
  ...
  {
    var count = layerNameInput.text.length;
    characterCountLabel.text = "Character Count: " + count;
    dialogTextReplacement();
  };
...
function loadItemsFromFile(filePath, callback) 
{
  var file = new File(filePath);
  file.open("r");
